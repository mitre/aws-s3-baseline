...
control "Public_S3_Objects" do
  ...
  tag check:
        "Review your AWS console and note if any S3 bucket objects are set to
        'Public'. If any objects are listed as 'Public', then this is
        a finding."

  tag fix:
        "Log into your AWS console and select the S3 buckets section. Select
  ...
  if aws_s3_buckets.bucket_names.empty?
        ...
        failure_message =
          (
            if public_objects.count > 1
              "#{public_objects.join(", ")} are public"
            else
              "#{public_objects.join(", ")} is public"
            end
          )
    ...
    aws_s3_buckets.bucket_names.each do |bucket|
      ...
      describe bucket.to_s do
        it "should not have any public objects" do
          failure_message =
            "\t- #{public_objects_multi.join("\n\t- ")} \n\tis public"
          failure_message =
            "\t- #{public_objects_multi.join("\n\t- ")} \n\tare public" if public_objects_multi.count >
            1
